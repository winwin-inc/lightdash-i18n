...
const ProjectAccessRowV2: FC<Props> = ({
    ...
    const { highestRole, highestRoleType } = useMemo(() => {
        if (
            ...
            return {
                ...
                highestRoleType: `${t(
                    'components_project_access_row_v2.group',
                )} ${userGroupAccess.group.name}`,
            };
        }
        return {
            highestRole: organizationRole,
            highestRoleType: t('components_project_access_row_v2.organization'),
        };
    ...
    const userRoleSummary = useMemo(() => {
        return (
            <Stack>
                <Text fw={300}>
                    {t('components_project_access_row_v2.organization_role')}:{' '}
                    <Text fw={600} span>
                ...
                {userGroupAccess?.access.roleId ? (
                    <Text fw={300}>
                        {t('components_project_access_row_v2.group')}{' '}
                        <Text fw={600} span>
                            {userGroupAccess.group.name}
                        </Text>
                        :{' '}
