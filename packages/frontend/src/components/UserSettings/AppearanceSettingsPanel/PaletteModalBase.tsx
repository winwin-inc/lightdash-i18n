        ...
        validate: {
            name: (value) => {
                if (!value || value.trim().length < 3) {
                    return t(
                        'components_user_settings_appearance_settings_panel_palette_modal.validate.name',
                    );
                }
                if (existingPaletteNames.includes(value)) {
                    return t(
                        'components_user_settings_appearance_settings_panel_palette_modal.validate.unique',
                    );
                }
                return null;
            },
            colors: (value) =>
                value.every((c) => c.startsWith('#'))
                    ? null
                    : t(
                          'components_user_settings_appearance_settings_panel_palette_modal.validate.invalid_colors',
                      ),
        },
    });
                ...
                >
                    ...
                    >
                        <Stack p="sm" spacing="xs">
                            <TextInput
                                label={t(
                                    'components_user_settings_appearance_settings_panel_palette_modal.name.label',
                                )}
                                placeholder={t(
                                    'components_user_settings_appearance_settings_panel_palette_modal.name.placeholder',
                                )}
                                required
                                size="sm"
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        {showAllColors
                                            ? t(
                                                  'components_user_settings_appearance_settings_panel_palette_modal.colors.show_fewer_colors',
                                              )
                                            : t(
                                                  'components_user_settings_appearance_settings_panel_palette_modal.colors.show_all_colors',
                                              )}
                                    </Button>
                                </Box>
                    ...
                    >
                        <Group position="right" spacing="xs">
                            <Button variant="default" h={32} onClick={onClose}>
                                {t(
                                    'components_user_settings_appearance_settings_panel_palette_modal.cancel',
                                )}
                            </Button>
                            <Button
                                h={32}
