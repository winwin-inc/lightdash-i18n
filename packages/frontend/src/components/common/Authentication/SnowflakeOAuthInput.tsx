...
export const SnowflakeOAuthInput: FC<Props> = ({ onAuthenticated }) => {
    ...
    if (isAuthenticated) {
        return (
            <Text mt="0" color="gray" fs="xs">
                {t(
                    'components_project_connection_warehouse_form.snowflake.authentication.you_are_connected',
                )}{' '}
                <Anchor
                    href="#"
                    onClick={() => {
                ...
                >
                    {t(
                        'components_project_connection_warehouse_form.snowflake.authentication.reauthenticate',
                    )}
                </Anchor>
            </Text>
        );
    ...
    return (
        ...
        >
            {t(
                'components_project_connection_warehouse_form.snowflake.authentication.sign_in_with_snowflake',
            )}
        </Button>
    );
};
