...
// STUB: full filter label helpers (getConditionalRuleLabel*) not ported ù show field ids.
...
const DataAppTile: FC<Props> = (props) => {
    ...
    // whose target field isn't in a given query's explore ù??an app may
    ...
    // invalidateCache so those re-fired queries bypass the warehouse cache ù??
    ...
    // gone ù??`useGetApp` would 404 anyway, but bypassing the request avoids
    ...
    // Authoritative across ALL versions ù??the ready version may be older than
    ...
    // (AppPreviewTest.tsx) ù??same space-aware ability check.
    const { user } = useApp();
    ...
    // reloads and its mount-time metric queries re-fire ù??by then the bridge
    ...
    // with filters ù? tiles and 502 on large dashboards.
