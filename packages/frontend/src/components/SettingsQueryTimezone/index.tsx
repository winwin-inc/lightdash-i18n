    ...
    return (
        <form onSubmit={form.onSubmit(onSubmit)}>
            <Stack gap="md">
                <TimeZonePicker
                    label={t(
                        'components_settings_query_timezone.form.timezone_label',
                    )}
                    variant="default"
                    maw="100%"
                    searchable
                    clearable
                    placeholder={t(
                        'components_settings_query_timezone.form.timezone_placeholder',
                    )}
                ...
                {showFilterInputsToggle && (
                    <Switch
                        label={t(
                            'components_settings_query_timezone.form.filter_toggle_label',
                        )}
                        description={t(
                            'components_settings_query_timezone.form.filter_toggle_description',
                        )}
            ...
            <Flex justify="flex-end" gap="sm" mt="sm">
                <Button
                    ...
                    loading={isLoading}
                >
                    {t('components_settings_query_timezone.form.update')}
                </Button>
            </Flex>
        </form>
    );
    ...
    const handleSubmit = useCallback(
        async (values: QueryTimezoneFormValues) => {
            ...
            try {
                ...
                showToastSuccess({
                    title: t('components_settings_query_timezone.tips.success'),
                });
            } catch (e) {
                const errorMessage = isApiError(e)
                    ...
                    : getErrorMessage(e);
                showToastError({
                    title: t('components_settings_query_timezone.tips.failed'),
    ...
    return (
        <Stack gap="sm" pos="relative">
            ...
            <SettingsGridCard>
                <Stack gap="xs">
                    <Title order={5}>
                        {t('components_settings_query_timezone.title')}
                    </Title>
                    <Text c="ldGray.6" fz="sm">
                        {timezoneSupportEnabled
                            ? t(
                                  'components_settings_query_timezone.description_enabled',
                              )
                            : t(
                                  'components_settings_query_timezone.description_disabled',
                              )}
                    </Text>
                </Stack>
                <div>
