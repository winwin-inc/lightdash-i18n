...
// centered-composer ù??split-sidebar handoff). flushSync forces React to
...
const AppGenerate: FC = () => {
    ...
    // (urlAppUuid undefined) ù??once we're editing an existing app the space
    ...
    // Editor handle (TipTap-based) ù??replaces the previous controlled
    ...
    // 'pick'    ù??show the 4 template cards (replaces the empty state)
    // 'confirm' ù??wizard collapses; the textarea takes over. Picking any
    //             template lands here directly ù??clarifying questions
    ...
    // Capability flag ù??flipped to true when the iframe SDK announces the
    ...
    // Screenshot button stays hidden ù??they keep working as before.
    ...
    // ù??the panel-group layout is never touched, so expanding restores the
    ...
    // user re-opens it from the dots menu ù??same model as preview, for
    ...
    // Maps prompt text ù??image preview URL so the thumbnail survives the
    // localù??server message transition (localMessages get cleared when server
    // version data arrives, but the ref persists).
    const sentImagesByPrompt = useRef(new Map<string, string[]>());
    // Maps prompt text ù??non-image attachment chips, same lifecycle as above
    const sentFilesByPrompt = useRef(new Map<string, ChatAttachedFile[]>());
    // Maps prompt text ù??chart names so they survive the localù??server transition
    const sentChartsByPrompt = useRef(new Map<string, ChatChart[]>());
    // Maps prompt text ù??connection names so they survive the localù??server transition
    const sentConnectionsByPrompt = useRef(new Map<string, ChatConnection[]>());
    // Maps prompt text ù??dashboard name so it survives the localù??server transition
    ...
    // slug ù??never seed it here: only `useGetApp` accepts a slug, and every
    ...
    // Connections already linked to this app ù??shown as an "available" pill so
    ...
    // vs. the post-submit URL update (undefined ù??newUuid).
    ...
    useEffect(() => {
        ...
        // Post-submit redirect: undefined ù??new uuid. Don't clear state.
    ...
    // currently displayed ù??interactive state included. Must stay in the
    ...
    // Latest build activity stands in for "last modified" ù??apps have no
    ...
    // an existing app ù??space editors/admins inherit manage on its data app.
    ...
    // Excludes `isSubmitting` (client-side upload ù??too early to claim
    ...
    // Lookup table: version number ù??full version summary. Used to retrieve
    ...
    // whose `submittedAtVersion` is older than `maxHistoryVersion` ù??see
    ...
    // accumulate versions across fetches in `versionCacheRef` ù??so even if the
    ...
    // building ù??preview defaults to this unless the user pins an older one.
    ...
    // version (any status ù??a still-building version's model is already a
    ...
    //   version's resources ù??survives org-default changes and theme
    ...
    // `effectivePinnedVersion` below ù??no useEffect+setState chain needed
    ...
    //  - pin from a different app (user navigated away) ù??ignore.
    //  - a newer ready version exists than at pin time ù??ignore (the user
    //    just authored a fresh prompt; show them the result, not the stale
    //    review state).
    //  - the pinned version is no longer in the ready set ù??ignore.
    ...
    // Upgrade offer for the header menu. Keyed to the latest ready bundle ù??
    // the one an upgrade would rebuild from ù??not to whatever version the
    ...
    // than `number | null` ù??TS won't carry inline ternary narrowing into a
    ...
    // in-flight entries to a terminal "interrupted" state ù??the iframe that
    ...
    useEffect(() => {
        ...
        if (prev === null) return; // Initial render ù??nothing to clean up.
        if (persistLogs) {
            interruptInFlightQueries();
            interruptInFlightRequests();
    ...
    const scrollToBottom = useCallback(() => {
        ...
        // on a child ù??scrollIntoView also scrolls outer ancestors (incl. the
    ...
    // Two paths: creating a brand-new app (no urlAppUuid) ù??check `create`;
    // editing an existing one ù??check `manage` with the app's space access
    // and creator context (so space editors and the creator of a personal
    // app both match).
    // Wait for the app to load before deciding the existing-app case ù??
    ...
    const handleSubmit = async () => {
        ...
        if (
            ...
            return;
        // Inspector references travel as their own lines after the typed text ù??
        ...
        // template like the others ù??it flows through the same clarify + build
        ...
        try {
            ...
            // Capture preview URLs / file chips before clearing ù??they stay in
            // the message bubble. Also store in the refs so they survive the
            // localù??server transition.
            ...
            setLocalMessages((prev) => [
                ...prev,
                {
                    ...
                    // bubble is dropped by `mergeChatMessages` ù??even if the
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        {!isViewingOlderVersion && (
                            <Box className={classes.chatInputArea}>
                                {/* No `accept` ù??any text-based file is
