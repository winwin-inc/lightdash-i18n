...
export default function AppPreviewTest() {
    ...
    // user doesn't have access â€?we surface that as the error state below.
    ...
    // Latest build activity stands in for "last modified" â€?apps have no
    ...
    // No successful build yet â€?distinguish "still building" (poll so the
    ...
    // clears stale entries on version navigation â€?this component re-renders
    ...
    // results cache â€?the initial load still serves cached results fast.
    ...
    // Live-preview capture for the move modal's thumbnail checkbox â€?same
    ...
    // App data is loaded â€?always render the header chrome so users can still
    // navigate (rename, delete, "Continue building", â€? when there's nothing
    // to preview yet.
    let body: ReactNode;
    if (!explicitVersion && hasNoReadyVersion) {
        // Deliberately neutral: don't surface build failures or their status
        // messages here â€?that's builder territory and too technical (and
