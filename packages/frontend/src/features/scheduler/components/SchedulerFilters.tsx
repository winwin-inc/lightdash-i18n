...
const FilterItem: FC<SchedulerFilterItemProps> = ({
    ...
    return (
        <Group spacing="xs" align="flex-start" noWrap>
            <Tooltip
                label={t('feature_scheduler_filters.reset_filter')}
                fz="xs"
                disabled={!hasChanged}
            >
                <ActionIcon
            ...
            <Stack key={dashboardFilter.id} spacing="xs" w="100%">
                <Group spacing="xs">
                    ...
                    <>
                        ...
                        {tilesWithFilter && tilesWithFilter.length > 0 && (
                            <Tooltip
                                label={`${t(
                                    'feature_scheduler_filters.applies_to',
                                )}: ${tilesWithFilter.join(', ')}`}
                                fz="xs"
                                multiline
                                w={200}
                            >
                                <Text fz="xs" color="gray.6" span>
                                    {`${t(
                                        'feature_scheduler_filters.applies_to',
                                    )} ${tilesWithFilter.length} ${t(
                                        'feature_scheduler_filters.tiles',
                                    )}`}
                                </Text>
                            </Tooltip>
                        )}
                ...
                {!isEditing && hasChanged && (
                    <Text fz="xs" color="gray.6">
                        {t('feature_scheduler_filters.unsaved_changes')}
                    </Text>
                )}
