...
type Props = {
    ...
    opened: boolean;
    onClose: () => void;
    /** Called after a successful move, before the modal closes ù?for
     ...
     *  the app exactly as the user sees it ù?including any interactive state
 ...
 * table so the move flow ù?and the app's ResourceViewItem shape it needs ù?
 * live in one place.
 *
 ...
 * an existing thumbnail ù?"Replace app thumbnail" instead of "Include app
 * thumbnail" ù?so the overwrite is explicit. The capture source is the
...
export const MoveAppToSpaceModal: FC<Props> = ({
    ...
    // (include vs replace) ù?a 404 here just means there's none yet. The
    ...
    // not depend on the async hasThumbnail check ù?a derived default would
    ...
    // Invisible fallback preview of the latest ready version ù?only mounted
    ...
    // Ref (not state): only the confirm handler reads it, by polling ù?no
    ...
    // Flips when the modal closes (or unmounts ù?every host renders it
    ...
    const captureFromFallbackPreview = async (): Promise<File> => {
        ...
        // Async poll ù?each iteration yields, so the event loop is never
        ...
        if (!screenshotAvailableRef.current || !capture) {
            throw new Error(
                "This app version doesn't support screenshot capture ù?iterate on it once to update it",
            );
        }
        return capture();
    };
    ...
    const captureAndUploadThumbnail = async () => {
        ...
        try {
            ...
            // Cancelled mid-capture ù?the failure is expected, stay quiet.
            if (closedRef.current) return;
            // The move is the primary action ù?a failed capture only warns.
            showToastWarning({
            ...
            <TransferItemsModal
                ...
                onConfirm={async (targetSpaceUuid) => {
                    ...
                    if (hasReadyVersion && includeThumbnail) {
                        ...
                        // Cancelled while capturing ù?don't move an app the
                        // user backed out of.
                        if (closedRef.current) return;
                    }
                    await contentAction({
