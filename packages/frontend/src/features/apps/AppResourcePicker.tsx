 ...
 * screenshot support â€?an always-present but permanently dead control reads as
...
type ModelOption = {
    ...
    // below, these form a capability spectrum (premium â†?balanced â†?budget)
    // so the trade-off is legible at a glance.
    tagline: string;
    isDefault?: boolean;
};

// Order: capability descending â€?Opus (highest quality) â†?Sonnet (default) â†?
 ...
 * Click â†?enables sample data; the pill then shows an inline database icon
 ...
 * Click â†?disables sample data; the pill reverts to plain and the
...
const InlineDataToggle: FC<{
    ...
    <Tooltip
        label={`Sample data included â€?click to remove.${tooltipSuffix ?? ''}`}
        multiline
        w={260}
        withArrow
    >
...
const AddLinkButton: FC<{ onClick: () => void; disabled?: boolean }> = ({
    ...
    <Tooltip
        label="Link live â€?run this chart by reference so the app updates when the chart changes in Lightdash."
        multiline
        w={260}
        withArrow
    >
...
const InlineLinkToggle: FC<{ onClick: () => void; disabled?: boolean }> = ({
    ...
    <Tooltip
        label="Linked live â€?click to unlink (revert to a copied query)."
        multiline
        w={260}
        withArrow
    >
 ...
 * `QueryPickerView`; only the selection model differs â€?single-select, so
...
export const ConnectionPickerView: FC<{
    ...
    // Project Settings â†?Data app connections page uses.
    const { user } = useApp();
...
/**
 * Opens the external-connection picker directly â€?used by surfaces that
