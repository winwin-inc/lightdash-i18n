...
type Props = {
    ...
    /** Data-lineage ("Inspect data") toggle â€?only shown on the Queries tab.
     *  When `onToggleLineage` is omitted, the toggle is hidden. */
    lineageEnabled?: boolean;
    lineageAvailable?: boolean;
    /** The bundle's manifest reports the lineage capability â€?so an
    ...
    /** Persist preference + handler â€?when omitted, the "Persist" switch is
    ...
    return (
        ...
        >
            <Group
                ...
                onClick={toggle}
            >
                {/* Both tabs are always shown â€?the Requests tab stays visible
                        ...
                        {onToggleLineage && activeTab === 'queries' && (
                            <Tooltip
                                label={
                                    !lineageAvailable
                                        ? lineageSupportedBySdk
                                            ? "Inspect data isn't wired into this app yet â€?ask the agent to add it"
                                            : 'Inspect data is not available in this app version â€?upgrade the app to enable it'
                                        : lineageEnabled
                                          ? 'Inspect data: on'
                                          : 'Inspect data'
                                }
