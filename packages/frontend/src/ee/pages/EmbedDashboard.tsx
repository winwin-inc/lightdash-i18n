...
const EmbedDashboard: FC<{ embedToken: string }> = ({ embedToken }) => {
    ...
    if (!projectUuid) {
        return (
            <div style={{ marginTop: '20px' }}>
                <SuboptimalState title={t('ai_embed_dashboard.missing_project_uuid')} />
            </div>
        );
    }
    if (dashboardError) {
    ...
    if (!dashboard) {
        return (
            <div style={{ marginTop: '20px' }}>
                <SuboptimalState title={t('ai_embed_dashboard.loading')} loading />
            </div>
        );
    }
        ...
        return (
            <div style={{ marginTop: '20px' }}>
                <SuboptimalState
                    ...
                    description={t('ai_embed_dashboard.empty_dashboard_description')}
                />
            </div>
        );
    }
                    ...
                    <div key={tile.uuid}>
                        {tile.type === DashboardTileTypes.SAVED_CHART ? (
                          ...
                          DashboardTileTypes.SEMANTIC_VIEWER_CHART ? (
                            ...
                            assertUnreachable(
                                tile,
                                t('ai_embed_dashboard.dashboard_tile_type_not_recognised'),
                            )
                        )}
                    </div>
                ))}
    ...
    if (!embedToken) {
        return (
            <div style={{ marginTop: '20px' }}>
                <SuboptimalState
                    icon={IconUnlink}
                    title={t('ai_embed_dashboard.embed_link_not_valid')}
                />
            </div>
        );
    }
