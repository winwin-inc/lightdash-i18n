            ...
            <Table.Td>
                <Tooltip
                    ...
                    label={
                        <Text size="xs">
                            {t(
                                'ai_copilot_chat_elements_evals_run_results.started',
                            )}{' '}
                            {createdAt}
                            {run.completedAt && (
                                <>
                                    <br />
                                    {t(
                                        'ai_copilot_chat_elements_evals_run_results.completed',
                                    )}{' '}
                                    {completedAt}
                                </>
                            )}
    ...
    return (
        <Stack gap="xs">
            <Title order={5} c="gray.9" fw={500}>
                {t('ai_copilot_chat_elements_evals_run_results.previous_runs')}
            </Title>
            ...
            {!runs || runs.length === 0 ? (
                ...
                >
                    ...
                    <Title order={5}>
                        {t(
                            'ai_copilot_chat_elements_evals_run_results.no_evaluation_runs_yet',
                        )}
                    </Title>
                    <Text size="sm" c="dimmed" ta="center">
                        {t(
                            'ai_copilot_chat_elements_evals_run_results.evaluation_runs_description',
                        )}
                    </Text>
                </Paper>
            ) : (
                        ...
                        <Table highlightOnHover stickyHeader>
                            <Table.Thead>
                                <Table.Tr>
                                    <Table.Th>
                                        {t(
                                            'ai_copilot_chat_elements_evals_run_results.run_id',
                                        )}
                                    </Table.Th>
                                    <Table.Th>
                                        {t(
                                            'ai_copilot_chat_elements_evals_run_results.status',
                                        )}
                                    </Table.Th>
                                    <Table.Th>
                                        {t(
                                            'ai_copilot_chat_elements_evals_run_results.created',
                                        )}
                                    </Table.Th>
                                    <Table.Th>
                                        {t(
                                            'ai_copilot_chat_elements_evals_run_results.duration',
                                        )}
                                    </Table.Th>
                                </Table.Tr>
                            </Table.Thead>
