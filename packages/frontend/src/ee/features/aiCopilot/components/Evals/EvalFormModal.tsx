...
export const EvalFormModal: FC<Props> = ({
    ...
    const modalTitle =
        mode === 'create'
            ? t('ai_copilot_chat_elements_evals_form_modal.create_evaluation')
            : t('ai_copilot_chat_elements_evals_form_modal.edit_evaluation');
    const submitButtonText =
        mode === 'create'
            ? t('ai_copilot_chat_elements_evals_form_modal.create_evaluation')
            : t('ai_copilot_chat_elements_evals_form_modal.save_changes');
        ...
        <Group justify="space-between" w="100%">
            {mode === 'edit' && onDelete ? (
                <Button
                    ...
                    form="eval-form"
                >
                    {t('ai_copilot_chat_elements_evals_form_modal.delete')}
                </Button>
            ) : (
                <div />
            )}
            ...
            <Group gap="sm">
                <Button
                    ...
                    disabled={isLoading || isDeleting}
                >
                    {t('ai_copilot_chat_elements_evals_form_modal.cancel')}
                </Button>
                <Button
                    type="submit"
                    loading={isLoading}
        ...
        >
            <form id="eval-form" onSubmit={form.onSubmit(handleSubmit)}>
                <Stack gap="md">
                    <TextInput
                        label={t(
                            'ai_copilot_chat_elements_evals_form_modal.title.label',
                        )}
                        placeholder={t(
                            'ai_copilot_chat_elements_evals_form_modal.title.placeholder',
                        )}
                        required
                    ...
                    <Textarea
                        label={t(
                            'ai_copilot_chat_elements_evals_form_modal.description.label',
                        )}
                        placeholder={t(
                            'ai_copilot_chat_elements_evals_form_modal.description.placeholder',
                        )}
                        rows={3}
                    ...
                    <Box>
                        <Group justify="space-between" align="center" mb="sm">
                            <Text size="sm" fw={500}>
                                {t(
                                    'ai_copilot_chat_elements_evals_form_modal.questions',
                                )}
                            </Text>
                            <Button
                                variant="subtle"
                            ...
                            >
                                {t(
                                    'ai_copilot_chat_elements_evals_form_modal.add_question',
                                )}
                            </Button>
                        </Group>
