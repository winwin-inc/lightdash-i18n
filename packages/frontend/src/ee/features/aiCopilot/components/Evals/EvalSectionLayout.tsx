...
export const EvalSectionLayout: FC<EvalSectionLayoutProps> = ({ children }) => {
    ...
    if (isEvaluationsList) {
        breadcrumbItems.push(
            <Text key="evaluations" size="lg" fw={500}>
                {t('ai_copilot_chat_elements_evals_section_layout.evaluations')}
            </Text>,
        );
    } else {
        breadcrumbItems.push(
    ...
    if (isEvaluationDetail) {
        breadcrumbItems.push(
            <Text key="evaluation" size="lg" fw={500}>
                {evaluation?.title ||
                    t(
                        'ai_copilot_chat_elements_evals_section_layout.evaluation',
                    )}
            </Text>,
        );
    } else if (isRunDetail) {
    ...
    let pageDescription = t(
        'ai_copilot_chat_elements_evals_section_layout.description.part_1',
    );

    if (isEvaluationDetail) {
        pageDescription = t(
            'ai_copilot_chat_elements_evals_section_layout.description.part_2',
        );
    } else if (isRunDetail) {
        pageDescription = t(
            'ai_copilot_chat_elements_evals_section_layout.description.part_3',
        );
    }
    ...
    const RenderActionButton = () => {
        ...
        if (isEvaluationDetail) {
            return (
                ...
                >
                    {t(
                        'ai_copilot_chat_elements_evals_section_layout.run_evaluation',
                    )}
                </Button>
            );
        }
        ...
        if (isEvaluationsList) {
            return (
                ...
                >
                    {t(
                        'ai_copilot_chat_elements_evals_section_layout.create_evaluation',
                    )}
                </Button>
            );
        }
