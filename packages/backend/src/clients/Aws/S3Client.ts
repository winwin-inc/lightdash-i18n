...
export class S3Client {
    ...
    constructor({ lightdashConfig }: S3ClientArguments) {
        ...
        if (lightdashConfig.s3?.endpoint && lightdashConfig.s3.region) {
            ...
            Logger.info(
                `S3 request timeout configured: ${requestTimeout}ms (${
    ...
    private async uploadFile(
        ...
        try {
            await upload.done();
            return await this.getDownloadUrl(prefixedKey, urlOptions);
        } catch (error) {
    ...
    async getUploadPresignedUrl(
        ...
        try {
            ...
            if (error instanceof S3ServiceException) {
                ...
                Logger.error(
                    `Failed to generate upload presigned URL: ${getErrorMessage(
                        error,
                    )}`,
                );
            }
