...
/**
 * Parse static files version from environment variable or fallback to package.json version
 * Priority: STATIC_FILES_VERSION env var > package.json VERSION > undefined
...
/**
 * Parse CDN configuration from environment variables
...
const parseCdnConfig = (): LightdashConfig['cdn'] => {
    ...
    if (isSemverPrereleaseVersion(staticFilesVersion)) {
        console.log(
            `[CDN Config] Prerelease ${staticFilesVersion} ‚Ä?serving frontend from backend (skip CDN)`,
        );
        baseUrl = undefined;
    } else {
...
export type AppRuntimeConfig = {
     ...
     * previews are served same-origin ‚Ä?dev / pre-cutover behavior. The
     ...
     * back to E2B's implicit `default` tag ‚Ä?used as a transition state for
     ...
     * Lightdash CLI + Claude Code) ‚Ä?the Docker analog of the separate E2B
     ...
     * launches from ‚Ä?passed as `sourcesRef.diskImage.id`. Required only when
     * `azure-sandboxes`. */
     ...
     * git + Claude CLI + the generic skill only ‚Ä?no dbt venvs, no compile
     ...
     *
     * `0` (default) disables the check ‚Ä?deliberately off, matching the rest
     ...
     * that defaults on. The check FAILS CLOSED ‚Ä?if OSV can't be reached the
     * upload is rejected ‚Ä?so an instance whose backend has no egress to
     ...
     * warehouse row values are sent to the sandbox or the LLM ‚Ä?the per-chart
...
export type DataAppOtelConfig = {
    ...
    protocol: string;
    /** OTEL_TRACES_EXPORT_INTERVAL (ms) ‚Ä?short so spans flush before teardown. */
     ...
     * provider-specific behaviour out of this config ‚Ä?it only selects a path.
     */
...
const parseAppRuntimeConfig = (siteUrl: string): AppRuntimeConfig => {
    ...
    const sandboxSnapshotRetentionMs = process.env.SANDBOX_SNAPSHOT_RETENTION_MS
        ...
        : 7 * 24 * 60 * 60 * 1000;
    // Lambda MicroVMs run in eu-west-1 (Ireland) ‚Ä?the EU launch region.
    ...
    return {
        ...
        // release by the post-release workflow at the running version's tag ‚Ä?
        ...
        dependencyRegistryHosts: (
            ...
            .filter(Boolean)
            .map((host) => {
                // These feed the sandbox egress allowlist ‚Ä?fail loudly on
...
export const parseConfig = (): LightdashConfig => {
    ...
    return {
        ...
        smtp: process.env.EMAIL_SMTP_HOST
            ...
            : undefined,
        // Ëá™ÊâòÁÆ°ÔºöÁ°¨ÂÖ≥Èó≠ÂÆòÊñ?PostHog / RudderStackÔºåÂøΩÁï•Áõ∏ÂÖ≥ÁéØÂ¢ÉÂèòÈá?
        posthog: undefined,
        rudder: {
            writeKey: undefined,
        ...
        // Ëá™ÊâòÁÆ°ÔºöÁ°¨ÂÖ≥Èó≠ÂÆòÊñ?Intercom / Pylon ÂÆ¢ÊúçÁªÑ‰ª∂
        intercom: {
            appId: '',
            apiBase: '',
        },
