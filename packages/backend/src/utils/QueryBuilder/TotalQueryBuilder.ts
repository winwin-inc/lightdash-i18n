...
// Fork does not ship parseTableCalculationFunctions; treat known table-calc
// helpers / aggregates as non-totalable (same intent as upstream parser).
const NON_TOTALABLE_SQL_FN_PATTERN =
    /\b(pivot_index|pivot_offset|row_index|is_first_row_in_partition|is_last_row_in_partition|offset|total|sum|avg|average|count|min|max|percentile|median)\s*\(/i;
...
const getTotalableReferences = (calc: TableCalculation): string[] | null => {
    ...
    if (isSqlTableCalculation(calc)) {
        if (
            ...
            NON_TOTALABLE_SQL_FN_PATTERN.test(calc.sql)
        ) {
            return null;
        }
        try {
