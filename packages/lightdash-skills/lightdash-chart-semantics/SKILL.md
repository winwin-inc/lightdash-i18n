...
**分工：** 分支与门禁在 **[`lightdash-insight-router`](../lightdash-insight-router/SKILL.md)**（**[`ROUTER-SOP.md`](../lightdash-insight-router/ROUTER-SOP.md)**）；**[`lightdash-metric-query`](../lightdash-metric-query/SKILL.md)** 管 `run_semantic_metric_query` / 扁平 `run_metric_query`。本技能管**可视化语义**与 MCP 结果解读。
