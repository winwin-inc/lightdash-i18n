...
describe('Scheduler utils', () => {
    describe('getHumanReadableCronExpression', () => {
        test('Should convert human readable cron expression with UTC', async () => {
            expect(
                getHumanReadableCronExpression('* * * * *', 'UTC', 'en'),
            ).toEqual('every minute, every hour, every day');

            expect(
                getHumanReadableCronExpression('0 * * * *', 'UTC', 'en'),
            ).toEqual('every hour, every day');

            expect(
                getHumanReadableCronExpression('0 0 * * *', 'UTC', 'en'),
            ).toEqual('at 12:00 AM (UTC +00:00), every day');

            expect(
                getHumanReadableCronExpression('0 13 * * *', 'UTC', 'en'),
            ).toEqual('at 01:00 PM (UTC +00:00), every day');

            expect(
                getHumanReadableCronExpression('0 13 1 * *', 'UTC', 'en'),
            ).toEqual('at 01:00 PM (UTC +00:00), on day 1 of the month');

            expect(
                getHumanReadableCronExpression('0 13 1 1 *', 'UTC', 'en'),
            ).toEqual(
                'at 01:00 PM (UTC +00:00), on day 1 of the month, only in January',
            );

            expect(
                getHumanReadableCronExpression('0 13 1 1 1', 'UTC', 'en'),
            ).toEqual(
                'at 01:00 PM (UTC +00:00), on day 1 of the month, and on Monday, only in January',
            );

            expect(
                getHumanReadableCronExpression('* 13 1 * *', 'UTC', 'en'),
            ).toEqual(
