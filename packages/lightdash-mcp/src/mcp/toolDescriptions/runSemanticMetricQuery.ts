...
- 未传时回退 session（set_project）或环境变量 LIGHTDASH_PROJECT_UUID。
...
## 参数
- metricQuery（必填，string）：Explorer Metric Query JSON 字符串（可含 optional projectUuid / dashboardUuid）
- projectUuid（可选，覆盖 JSON 内字段）、dashboardUuid（可选，覆盖 JSON 内字段）、limit（可选，覆盖 JSON 内 limit）、invalidateCache、full
...
run_semantic_metric_query({
  ...
  metricQuery: "{\\"projectUuid\\":\\"<可选>\\",\\"dashboardUuid\\":\\"<可选>\\",\\"exploreName\\":\\"...\\",\\"dimensions\\":[...],\\"metrics\\":[...],\\"filters\\":{...},\\"limit\\":500}"
})
