...
- 使用仓库 **`skills/`** 目录，例如 `skills/lightdash-data-tools/SKILL.md` 作为**路由技能**（何时 list / get explore / run query）。同目录提供 **`quickstart.md`**（5 步上手）、**`examples.md`**（三则最短示例）、**`examples-advanced.md`**（自定义维度、时区、表计算等）；可按需增加子技能。
...
| 2026-03-26 | `skills/lightdash-data-tools` 文档拆分为 quickstart / examples / examples-advanced |
