...
一句话总结：根 spec 作为 **desktop**；可选 `lightdash.responsive.mobile` 在容器宽度低于 `breakpoint`（默认 768）时**整份替换**为 mobile spec。无配置时行为与改前完全一致。
...
| 字段 | 说明 |
|------|------|
| 根 spec | **desktop**，宽屏（容器宽度 ≥ breakpoint）时使用 |
| `lightdash.responsive.breakpoint` | 断点像素，默认 `768`；按**容器宽度**判断（非 window） |
| `lightdash.responsive.mobile` | 完整 Vega-Lite spec，窄屏时整份替换 desktop |
...
width >= breakpoint             → 根 spec（desktop）
width < breakpoint 且无 mobile  → 根 spec（fit 压缩，与现状相同）
width < breakpoint 且有 mobile  → mobile spec 整份替换
根 spec 为复合图                 → 忽略 mobile（开发环境 console 警告）
```

容器宽度来自 chart tile / Explorer 区域的 `ResizeObserver`，dashboard 半宽 tile 也会正确切换。
...
1. **改 PC 布局** → 只动根 spec（`layer` / `encoding`）
2. **改移动端** → 只动 `lightdash.responsive.mobile`
3. **`rewrite: true`** → 只在根 spec 写一次；窄屏时自动作用于 mobile spec
4. **红绿配色 expr** → desktop / mobile 两处保持一致（建议复制同一段）
5. **`breakpoint`** → 按实际 tile 宽度调整，不必固定 768

验证步骤：

1. Explorer 宽屏确认 desktop
2. DevTools 切到 375px 或缩窄 dashboard tile 确认 mobile
...
| 现象 | 可能原因 |
|------|----------|
| 窄屏仍显示 desktop 竖 bar | 容器宽度 ≥ breakpoint；或未配置 `mobile` |
...
| 路径 | 说明 |
|------|------|
| `packages/frontend/src/components/CustomVisualization/responsive/` | 解析、选 spec、layout 计算 |
