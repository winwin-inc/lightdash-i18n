...
1. **`list_explores`**（可选 `filtered: true`）→ 选定 explore 名；或用 **`find_explores`** 按关键词搜数据主题
2. **`find_fields`**（`table` + `fieldSearchQueries`）→ 从目录检索结果里拿到真实 **`fieldId`**
3. **`run_metric_query`** → 填 `exploreName`、`dimensions`、`metrics`、`filters`、`sorts`、`limit` 等
...
# 4. 重载 MCP，确认出现当前注册的 MCP tools（共 19 个，见 packages/lightdash-mcp/README.md）
```
...
- 重载 MCP 后应看到 **19** 个工具（15 个核心 + 4 个站点/已保存图表辅助；详见 `packages/lightdash-mcp/README.md`）

### 6. 验证

列出 MCP tools 或试调 `list_explores`；若 401/403，检查 PAT 与项目权限；HTTP 模式另查服务端是否监听、`url` 是否带 `/mcp`。
...
完整清单与参数见 **`packages/lightdash-mcp/README.md`**。以下为常用分组摘要（工具名**无前缀**，与源码注册一致）：

| 分组 | Tool 名 | 说明 |
|------|---------|------|
| 健康/组织 | `get_lightdash_version`、`list_projects` | 版本与健康、项目列表 |
| 会话 | `set_project`、`get_current_project` | 本会话默认项目 |
| 目录 | `list_explores`、`find_explores`、`find_fields` | 列表 / 关键词搜 explore / 在 table 内搜字段 |
| 内容 | `find_content`、`find_charts`、`find_dashboards`、`find_spaces`、`list_verified_content` | v2 内容搜索（混合或按类型固定） |
| 查询 | `search_field_values`、`run_sql`、`run_metric_query` | 枚举取值、SQL、指标查询 |
| 站点/图表 | `get_site_info`、`list_spaces`、`get_saved_chart`、`run_saved_chart` | 站点根 URL、空间、已保存图表 |
...
- **字段报错**：回到 `find_fields` / `find_explores` 核对 `fieldId`
