...
# 3. 二选一
#    A. stdio：项目根 .mcp.json 用 command + args 指向 dist/index.js（见下文）
#    B. HTTP：pnpm -F @lightdash/mcp start:http，客户端 .mcp.json 用 type + url + headers（见下文）

# 4. 重载 MCP，确认出现 lightdash_* tools
```

**技能包 / 仅分发 Skills 的用户**：优先使用 **HTTP MCP**（独立进程 + 客户端只配 URL），模板见 [skills/.mcp.json.example](../skills/.mcp.json.example)。
...
| 模式 | 入口 | 说明 |
|------|------|------|
| **stdio** | `dist/index.js` | 客户端用子进程拉起；适合本 monorepo |
...
```json
{
  "mcpServers": {
    "lightdash": {
      "command": "node",
      "args": ["packages/lightdash-mcp/dist/index.js"],
      "env": {
...
- `args` 相对**仓库根**；`env` 可省略（由 `packages/lightdash-mcp/.env` 提供）。
- 可不设 `LIGHTDASH_API_KEY`，改为每次 tool 传 `apiKey`。
...
- 重载 MCP 后应看到 `lightdash_list_explores`、`lightdash_get_explore`、`lightdash_run_metric_query`

### 5. Cursor

- **Settings → MCP** 添加 server，字段与上一节 JSON 一致
...
| 技能 | 路径 | 用途 |
|------|------|------|
| 路由 | [skills/lightdash-data-tools/SKILL.md](../skills/lightdash-data-tools/SKILL.md) | 何时用哪个 tool、标准顺序 |
| Metric Query | [skills/lightdash-metric-query/SKILL.md](../skills/lightdash-metric-query/SKILL.md) | filters、customDimensions、timezone 等 |

### 技能文件

| 文档 | 内容 |
|------|------|
| [quickstart.md](../skills/lightdash-data-tools/quickstart.md) | 5 步最快跑通 |
| [examples.md](../skills/lightdash-data-tools/examples.md) | 三则最短 JSON 示例 |
| [examples-advanced.md](../skills/lightdash-data-tools/examples-advanced.md) | 自定义维度、时区、表计算等 |
...
| `lightdash_run_metric_query` | 执行查询并等待结果 | `apiKey?`, `projectUuid`, `query`, `pageSize?`, `maxPollAttempts?`, `pollIntervalMs?` |

### v2 扩展（按需）

| Tool 名 | 前提 |
|---------|------|
| `lightdash_get_query_page` | 同一 `queryUuid` 拉取多页 |
| `lightdash_list_saved_charts` | 列出已保存图表 |
| `lightdash_search_field_values` | 搜索字段值 |
...
5. **`index.ts`**：stdio 入口
6. **`http.ts`**：Streamable HTTP 入口（Express + `StreamableHTTPServerTransport`）
7. **`config.ts`**：从环境变量加载配置
...
| [docs/lightdash-mcp-testing.md](./lightdash-mcp-testing.md) | 测试用例与环境（stdio / HTTP） |
...
| 日期 | 说明 |
|------|------|
| 2026-03-27 | 补充 stdio / Streamable HTTP 双模式、客户端示例与模块说明；技能包 HTTP 模板 |
