...
- 重载 MCP 后应看到 8 个工具（项目/内容/空间/保存图表 + explore/metric query）
...
| `lightdash_search_content` | 搜索看板/图表/空间 | `apiKey?`, `projectUuid`, `search?`, `contentTypes?`, `page?`, `pageSize?` |
| `lightdash_list_spaces` | 列出项目空间 | `apiKey?`, `projectUuid` |
| `lightdash_get_saved_chart` | 获取已保存图表详情 | `apiKey?`, `chartUuid` |
