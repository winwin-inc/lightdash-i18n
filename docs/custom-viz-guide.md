...
窄屏适配为 **opt-in**：根 spec 作 desktop，可选 `lightdash.responsive.mobile` 在容器宽度低于 breakpoint 时切换为 mobile spec。无配置时行为不变。

详细配置说明、完整示例与维护指南见 **[custom-viz-responsive.md](./custom-viz-responsive.md)**。
